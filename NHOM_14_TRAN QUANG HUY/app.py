...
class VoiceChatServer:
    ...
    async def handle_key_exchange(self, websocket, data):
        """Xử lý trao đổi khóa AES (Bước 2)"""
        try:
            target_id = data.get('target_id')
            target_ws = self.find_client_by_id(target_id)
            
            if not target_ws:
                await websocket.send(json.dumps({'type': 'error', 'message': 'Target client not found'}))
                return
            
            session_key = get_random_bytes(32)  # Tạo khóa AES-256 (32 bytes)
            sender_id = self.clients[websocket]['id']
            self.clients[websocket]['session_keys'][target_id] = session_key
            self.clients[target_ws]['session_keys'][sender_id] = session_key
            ...
            b64_encrypted_key = base64.b64encode(encrypted_session_key).decode()

            log_payload = {
                "metadata": metadata,
                "signed_info": b64_signature,
                "encrypted_aes_key": b64_encrypted_key
            }
            self.log_event({
            ...
            await target_ws.send(json.dumps({
                ...
                'from_id': sender_id,
                'signed_info': b64_signature,
                'encrypted_session_key': b64_encrypted_key,
                'metadata': metadata,
                'timestamp': timestamp
            }))
